...
Added attr_reader for the options passed to new Aegis::Roles.  The initialize method accepts it, but the developer never has access to it.  I'd like to pass :position so that I can sort my roles based on something other than randomness.  I found this useful for viewing user profiles and assigning the user's role.
